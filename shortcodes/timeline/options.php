		...
		'options' => array(
					...
					'design' => call_user_func( function () {
						if ( function_exists( 'fw_sc_design_picker_choices' ) ) {
							return array(
								...
								'choices' => fw_sc_design_picker_choices( 'timeline' ),
							);
						}
			...
			'group_style' => array(
				...
				'options' => array(
					...
					'howto_schema' => array(
						'type'  => 'switch',
						'label' => __( 'HowTo Schema (JSON-LD)', 'fw' ),
						'desc'  => __( 'Output HowTo structured data (each milestone becomes a step) so a step-by-step process is machine-readable to search engines and AI agents.', 'fw' ),
						'help'  => __( 'Only enable when the timeline is a genuine step-by-step how-to. The step name comes from each milestone Title and the step text from its Text.', 'fw' ),
