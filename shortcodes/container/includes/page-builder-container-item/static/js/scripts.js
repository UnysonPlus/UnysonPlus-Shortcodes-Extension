(function (fwe) {
	fwe.on('fw-builder:' + 'page-builder' + ':register-items', function (builder) {
		var PageBuilderContainerItem,
			PageBuilderContainerItemView,
		...
		PageBuilderContainerItemView = builder.classes.ItemView.extend({
			...
			render: function () {
				 ...
				 */
				fwEvents.trigger('fw:page-builder:shortcode:container:controls', {
			...
			editOptions: function (e) {
				...
				fwEvents.trigger('fw:page-builder:shortcode:container:modal:before-open', {
					modal: this.modal,
					model: this.model,
					builder: builder,
					flow: flow
			...
			cloneItem: function (e) {
				...
				clonedContainer = new PageBuilderContainerItem(attributes);

				triggerEvent(clonedContainer, 'clone-item:before');

				this.model.collection.add(clonedContainer, {at: index + 1});
				clonedContainer.get('_items').reset(_items);
			},
		...
		PageBuilderContainerItem = builder.classes.Item.extend({
			defaults: {
				type: 'container'
			},
			initialize: function() {
				this.view = new PageBuilderContainerItemView({
			...
			allowIncomingType: function (type) {
				// A container holds COLUMNS. Never a section-like item, and never another
				// container (containers don't nest — a section is the only place a container lives).
				if (type === 'container') {
					return false;
				}
				if (
			...
			allowDestinationType: function (type) {
				// A container may only be dropped INSIDE a section (section-like). Not at the
				// root, not inside a column, not inside another container.
				if (
					window.fwSectionLikeTypes
					&& typeof window.fwSectionLikeTypes.isSectionLike === 'function'
				) {
