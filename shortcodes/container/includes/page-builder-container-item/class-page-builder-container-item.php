...
class Page_Builder_Container_Item extends Page_Builder_Item
{
	...
	{
		$shortcode_instance = fw_ext('shortcodes')->get_shortcode('container');
		return $shortcode_instance->get_options();
	}

	private function get_shortcode_config()
	{
		return fw_ext( 'shortcodes' )->get_shortcode( 'container' )->get_shortcode_config();
	}
	...
	public function enqueue_static()
	{
		...
		// Reuse the SECTION builder-item CSS — the container's canvas item uses the same
		// `.custom-section` / `.pb-item` markup, so it inherits the identical look.
...
FW_Option_Type_Builder::register_item_type('Page_Builder_Container_Item');
