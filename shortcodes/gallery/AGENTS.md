...
`design_settings.design` ∈ `grid | masonry | justified | metro | carousel | polaroid | showcase` (default `grid`).
...
- **grid**: `columns` (1–6, def 3), `columns_tablet` (def 2), `columns_mobile` (def 1), `gap` (Gap-Scale slug, def `3`), `ratio` (`1-1|4-3|3-2|16-9|3-4|original`, def `4-3`).
- **masonry**: `columns` (def 3), `columns_tablet` (def 2), `gap`.
- **justified**: `row_height` (slider 120–420 px, def 220), `gap`.
- **metro**: `columns` (def 4), `gap`.
- **carousel**: `per_view` (def 3), `ratio` (def `4-3`), `gap`, `carousel_autoplay` (def no), `carousel_interval` (ms, def 4000), `carousel_pause_hover` (def yes), `carousel_loop` (def yes), `carousel_arrows` (def yes), `carousel_dots` (def yes).
- **polaroid**: `columns` (def 4), `columns_tablet` (def 2), `gap`, `tilt` (switch, def yes).
- **showcase**: `ratio` (def `4-3`), `thumb_position` (`bottom|left|right`, def bottom), `gap`.
- **cards**: `columns` (def 3), `columns_tablet` (def 2), `columns_mobile` (def 1), `gap`, `ratio` (def `4-3`). Forces caption-below.
...
- **thumbslider**: `ratio` (def `4-3`), `carousel_autoplay` (def no), `carousel_interval`, `carousel_pause_hover`, `carousel_loop`, `carousel_arrows`. Two synced Splides (main + nav).
- **coverflow**: `per_view` (def 3), `ratio` (def `4-3`), `gap`, `carousel_*` (autoplay def no, interval, pause_hover, loop def yes, arrows, dots). Splide `focus:center`; falloff via CSS `.is-active`.
...
- **filmstrip**: `per_view` (def 3), `ratio` (def `4-3`), `gap`. CSS scroll-snap, no library.
- **spotlight**: `feature_side` (`left|right`, def left), `columns` (thumb grid, def 3), `gap`. First image large + rest in a grid.
- **honeycomb**: `columns` (def 4), `gap`. Hexagon clip-path tiles.
- **accordion**: `row_height` (slider 180–560 px, def 340), `gap`. Flex panels that expand on hover; stacks on mobile.
- **flipcards**: `columns` (def 3), `columns_tablet` (def 2), `columns_mobile` (def 1), `gap`, `ratio` (def `4-3`). Hover-flip 3D; the WHOLE card is the lightbox anchor (custom front/back markup, not the shared tile).
