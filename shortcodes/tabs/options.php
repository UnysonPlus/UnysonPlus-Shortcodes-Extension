	...
	'tab_content' => [
		...
		'options' => [
			'tabs' => [
				...
				'template'      => '{{=tab_title}}',
				'popup-options' => array(
					// Pane — the tab's label + its panel content.
					'group_pane' => array(
						'type'    => 'group',
						'options' => array(
							'tab_title' => array(
							...
							'tab_content' => array(
								'type'  => 'wp-editor',
								'label' => __( 'Content', 'fw' ),
								'help'  => __( 'The panel shown when this tab is selected. Accepts HTML and shortcodes for richer content. Use the "Insert Snippet" button in the editor toolbar to drop in a full page-builder layout. In the "Media panel" layout it becomes the caption shown beneath the image.', 'fw' ),
							),
						),
					),
					// Media — the per-tab image used only by the "Media panel" layout.
					'group_media' => array(
						'type'    => 'group',
						'options' => array(
							'tab_image' => array(
					...
					// Tab options — decorations + per-tab state.
					'group_meta' => array(
						'type'    => 'group',
						'options' => array(
							'badge' => array(
		...
		'options' => [
			'group_layout' => [
				...
				'options' => [
					...
					'content_frame' => [
						'type'    => 'select',
						'label'   => __( 'Content Frame', 'fw' ),
						'desc'    => __( 'The box around the tab content panel.', 'fw' ),
						'help'    => __( 'Framed draws the classic bordered, rounded, padded panel with a background. Frameless removes the border, radius, background and side padding so the content sits flush (best when the content brings its own cards / layout) — a small top gap below the nav is kept.', 'fw' ),
						'value'   => 'framed',
						'choices' => [
							'framed'    => __( 'Framed (border + padding)', 'fw' ),
							'frameless' => __( 'Frameless (no box)', 'fw' ),
						],
					],
				],
			],
