...
| `text_align` | `image-picker` (`sc_alignment_field`, `inherit`) | `''` (Inherit) | **Text Alignment** — the CSS `text-align` for ALL content in the column (`''`/`left`/`center`/`right`). Renders a Bootstrap `text-*` class (`sc_alignment_class`) on the **OUTER** column wrapper; `''` emits nothing (inherit — additive, **no migration**). **Inherited property**, so it cascades to every nested block including an inner styling card — a DIFFERENT axis from `content_h` (flexbox positioning). The Site Converter sets this from a `text-center`/`text-right` cell/wrapper (`Mapper::cls_text_align` / `Stitch::wrapper_align`) |
