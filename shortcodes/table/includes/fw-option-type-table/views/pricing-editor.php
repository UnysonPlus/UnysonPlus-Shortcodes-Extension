...
/**
 * Legacy grid editor (pricing tables: button / switch / pricing rows).
 * Unchanged behavior from the original Unyson table option; only extracted out
 * of view.php so the orchestrator can toggle it against the new tabular editor.
 *
 * @var  array $option
 * @var  array $data
 */
...
$header_cell_template    = fw_ext( 'shortcodes' )->get_shortcode( 'table' )->get_declared_path() . '/includes/fw-option-type-table/views/cell-head-template.php';
$worksheet_cell_template = fw_ext( 'shortcodes' )->get_shortcode( 'table' )->get_declared_path() . '/includes/fw-option-type-table/views/cell-worksheet-template.php';
?>
