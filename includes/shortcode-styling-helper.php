...
if ( ! function_exists( 'sc_icon_svg_allowed' ) ) :
	/**
	 * Back-compat delegates — the SVG sanitiser now lives in framework/includes/svg-sanitize.php
	 * (fw_upw_*), so the CORE icon / image-mask option types are defended even on a core-only
	 * build. These sc_icon_* names are kept as thin wrappers for existing callers.
	 */
	function sc_icon_svg_allowed() { return function_exists( 'fw_upw_svg_allowed' ) ? fw_upw_svg_allowed() : array(); }
endif;

if ( ! function_exists( 'sc_icon_flatten_svg_css' ) ) :
	function sc_icon_flatten_svg_css( $markup ) { return function_exists( 'fw_upw_flatten_svg_css' ) ? fw_upw_flatten_svg_css( $markup ) : (string) $markup; }
endif;

if ( ! function_exists( 'sc_icon_sanitize_svg' ) ) :
	function sc_icon_sanitize_svg( $markup ) { return function_exists( 'fw_upw_sanitize_svg' ) ? fw_upw_sanitize_svg( $markup ) : ''; }
endif;
